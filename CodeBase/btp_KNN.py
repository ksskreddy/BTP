...
n = len(motion_flows1)
m = len(motion_flows3)
dtw_array_test = np.zeros((m,n));
p = 0
for i in range(m):
    for j in range(n):
        dtw_array_test[i][j], path1 = fastdtw(motion_flows3[i], motion_flows1[j], dist=frame_diff)
        print(p)
        p +=1

act1 = np.array(actual_output1)        
ans = act1[np.argmin(dtw_array_test,axis=1)]
